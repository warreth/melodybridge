...
1. The included `compose.yml` has a `flaresolverr` service. Start it
   with `docker compose up -d`.
2. In Settings, set the Cloudflare solver URL to
   `http://flaresolverr:8191` (or `http://127.0.0.1:8191` outside
   Docker).
